...
public class SqliteQueryExecutionEngineTests
{
    ...
    public async Task ExecuteAsync_WithWrongKindContinuationToken_ThrowsArgumentException()
    {
        await AssertTokenPayloadRejectedAsync(
            """{"Kind":"not-sqlite","OrderValues":[{"Field":"Age","Direction":"desc","Value":24}]}""",
    ...
    {
        await AssertTokenPayloadRejectedAsync(
            """{"Kind":"sqlite-seek-v1","OrderValues":[]}""",
    ...
    {
        await AssertTokenPayloadRejectedAsync(
            """{"Kind":"sqlite-seek-v1","OrderValues":[{"Field":"Name","Direction":"desc","Value":24}]}""",
            "order values mismatch");
    }
    ...
    {
        await AssertTokenPayloadRejectedAsync(
            """{"Kind":"sqlite-seek-v1","OrderValues":[{"Field":"Age","Direction":"asc","Value":24}]}""",
            "order values mismatch");
    }
    ...
    {
        await AssertTokenPayloadRejectedAsync(
            """{"Kind":"sqlite-seek-v1","OrderValues":[{"Field":"Age","Direction":"desc","Value":{"x":1}}]}""",
            "non-scalar");
    }
    ...
    {
        var engine = new SqliteQueryExecutionEngine(executor: new FakeNameOnlyExecutor());
        var source = new QuerySource(ProviderName.Sqlite, "Employees");
        using var connection = new FakeConnection();
    ...
    private sealed class FakeNameOnlyExecutor : ISqliteQueryExecutor
    {
        public Task<IReadOnlyList<T>> ExecuteRowsAsync<T>(
            IDbConnection connection,
            SqliteSqlTranslationResult translation,
    ...
    private sealed class FakePagedExecutor(
        ...
        long count,
        string seekMarker = "\"t\".\"Age\" <") : ISqliteQueryExecutor
    {
    ...
    private sealed class FakeDictionaryPagedExecutor : ISqliteQueryExecutor
    {
        private int _calls;
