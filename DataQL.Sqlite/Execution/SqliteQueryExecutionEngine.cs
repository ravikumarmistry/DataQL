...
public sealed class SqliteQueryExecutionEngine
{
    private const string SqliteContinuationTokenKind = "sqlite-seek-v1";

    private readonly QueryProcessor _processor;
    private readonly SqliteSqlTranslator _translator;
    private readonly ISqliteQueryExecutor _executor;
    ...
    public SqliteQueryExecutionEngine(
        ...
        SqliteSqlTranslator? translator = null,
        ...
        IProviderCapabilityValidator? capabilityValidator = null)
    {
        ...
        _translator = translator ?? new SqliteSqlTranslator();
        _executor = executor ?? new SqliteQueryExecutor();
    ...
    private SqliteContinuationPayload? DecodeSeekTokenOrThrow(
        ...
        IReadOnlyList<OrderClause> order)
    {
        ...
        if (!string.Equals(envelope.Provider, ProviderName.Sqlite, StringComparison.OrdinalIgnoreCase))
        {
        ...
        try
        {
            var payload = JsonSerializer.Deserialize<SqliteContinuationPayload>(envelope.ProviderToken);
            if (payload is null || !string.Equals(payload.Kind, SqliteContinuationTokenKind, StringComparison.Ordinal))
            {
    ...
    {
        var orderValues = new List<SqliteOrderValuePayload>(order.Count);
        foreach (var sort in order)
        {
            ...
            orderValues.Add(new SqliteOrderValuePayload
            {
                Field = field,
                Direction = direction,
        ...
        var envelope = new ContinuationTokenEnvelope
        {
            ...
            ProviderToken = JsonSerializer.Serialize(new SqliteContinuationPayload
            {
                Kind = SqliteContinuationTokenKind,
    ...
    private static FilterExpression BuildSeekFilterOrThrow(IReadOnlyList<OrderClause> order, SqliteContinuationPayload payload)
    {
    ...
    {
        if (!string.Equals(source.Provider, ProviderName.Sqlite, StringComparison.OrdinalIgnoreCase))
        {
            throw new ArgumentException("Source provider is not valid for Sqlite.", nameof(source));
        }

        if (string.IsNullOrWhiteSpace(source.Name))
        {
            throw new ArgumentException("Source name (table) is required for Sqlite execution.", nameof(source));
        }
    }

    private static string ResolveSqliteTableName(QuerySource source)
    {
        var split = source.Name.Split('.', 2, StringSplitOptions.TrimEntries | StringSplitOptions.RemoveEmptyEntries);
        if (split.Length == 2)
        {
    ...
    private static SqliteSqlTranslationResult ApplyLimitPlusOne(SqliteSqlTranslationResult translation, int limit)
    {
        if (translation.Sql.Contains(" LIMIT " + limit, StringComparison.Ordinal))
        {
            return new SqliteSqlTranslationResult
            {
                Sql = translation.Sql.Replace(" LIMIT " + limit, " LIMIT " + (limit + 1), StringComparison.Ordinal),
    ...
    private sealed class SqliteContinuationPayload
    {
        public string Kind { get; init; } = string.Empty;
        public IReadOnlyList<SqliteOrderValuePayload> OrderValues { get; init; } = [];
    }

    private sealed class SqliteOrderValuePayload
    {
