...
public sealed class SqliteQueryExecutor(
    ILogger<SqliteQueryExecutor>? logger = null) : ISqliteQueryExecutor
{
    private readonly ILogger<SqliteQueryExecutor>? _logger = logger;
    ...
    private void LogQuery(string queryType, string sql, IReadOnlyDictionary<string, object?> parameters)
    {
        ...
        _logger.LogInformation(
            "DataQL SQLite {QueryType} query: {Sql} | Parameters: {Parameters}",
            queryType,
            sql,
